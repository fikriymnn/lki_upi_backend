const StockOut = require('../../model/inventory/stock_out_model')
...
const stock_out_controller = {

   // ==============================
   // GET ALL + GET BY ID
   // ==============================
   get_stock_out: async (req, res) => {
      try {
         const { id } = req.params

         // GET BY ID
         if (id) {
            const data = await StockOut.findOne({ _id: id })
               .populate({
                  path: 'item_id',
         ...
         const total_data = await StockOut.countDocuments(filter)
   ...
   add_stock_out: async (req, res) => {
      try {
         ...
         if (previous_stock < parseInt(body.quantity)) {
            return res.status(200).json({
               success: false,
               status: 400,
               message: `Stok tidak mencukupi. Stok saat ini: ${previous_stock}`
            })
         }
   ...
   // DELETE + ROLLBACK
   // ==============================
   delete_stock_out: async (req, res) => {
      try {
         const { id } = req.params
         ...
         // Rollback stok item — tambah kembali
         ...
         if (item) {
            const rolled_back = item[field_stok] + data.quantity
         ...
         await StockOut.deleteOne({ _id: id })

         return res.status(200).json({
            success: true,
            message: 'Stock out berhasil dihapus dan stok telah di-rollback'
         })
      } catch (err) {
         return res.status(500).json({
            success: false,
...
module.exports = stock_out_controller